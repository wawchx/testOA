...
(function() {
	...
	(function() {
		...
		var mEvent=function(e) {
			...
			var mLeft = rootEl.scrollWidth - ym_win.offsetWidth;
			var mTop = rootEl.scrollHeight - ym_win.offsetHeight;
			sLeft = sLeft > mLeft ? mLeft : sLeft;
			sTop = sTop > mTop ? mTop : sTop;
		...
		var getPos=function(f){
			var sLeft = rootEl.scrollWidth - ym_win.offsetWidth;
			var sTop = rootEl.scrollHeight - ym_win.offsetHeight;
			sLeft = sLeft < 0 ? 0 : sLeft;
			sTop = sTop < 0 ? 0 : sTop;
			var pos=[sLeft, sTop];
		...
		var winVisible = function(visible) {
			...
			ym_body.innerHTML = !curCfg.iframe ? ('<div class="ym-content">' + curCfg.message + '</div>') : "<iframe id='ym_iframe' width='100%' height='100%' border='0' frameborder='0' src=''></iframe>"; //内容
		...
		var init = function() {
			...
			winVisible();
			if (curCfg.iframe) {
				if(curCfg.message.indexOf("?")!=-1){
					document.getElementById("ym_iframe").src=curCfg.message+"&"+Math.random();
				}else{
					document.getElementById("ym_iframe").src=curCfg.message+"?"+Math.random();
				}
			}
			
			//if(curCfg.iframe && navigator.userAgent.indexOf("MSIE 6.0") > -1){
				//解决IE6.0下动态创建iframe无法加载页面的问题
				//ym_iframe.location.reload();
			//}
		}; //销毁
