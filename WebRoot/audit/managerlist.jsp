				...
				<div class="querysheet">
					...
					<display:table name="resultList" id="row" class="mysheet" requestURI="/audit/findauditlist.action" sort="external" pagesize="15" partialList="true" size="resultSize">
						...
						<display:column title="操作" headerClass="last" class="last">
							<s:if test="#attr.row.status eq @com.duker.mygift.constant.DList$AuditInfo$status@going">
							<a href="javascript:auditinfo('${row.auditId}',4)">直接发布</a>
							<a href="javascript:auditinfo('${row.auditId}',5)">已通过</a>
							</s:if>
						</display:column>
					</display:table>
