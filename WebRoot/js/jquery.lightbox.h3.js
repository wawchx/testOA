...
(function($) {
	 ...
	 *
	 */
	$.fn.lightBox = function(settings,params) {
		...
		settings = jQuery.extend({
			...
			//imageBtnClose:			globalContextPath+'/images/common/lightbox/lightbox-btn-close.gif',		// (string) Path and the name of the close btn
			imageBtnClose:			'',
		...
		function init(){
			...
			settings.imageArray = settings.largeImageArray;
			
			_set_image_to_view();
			
			//_set_interface_bind(null,null);
		}
		/**
		...
		function _set_image_to_view() { // show the loading
			...
			objImagePreloader.onerror  = function() {
				$('#lightbox-image').attr('src',settings.imageArray[settings.activeImage][0]);
				//加载失败,重置为原始大小
				_resize_container_image_box(250,250);
				objImagePreloader.onload=function(){};
			};
			
			objImagePreloader.src = settings.imageArray[settings.activeImage][0] +  "?" + new Date().getTime();
