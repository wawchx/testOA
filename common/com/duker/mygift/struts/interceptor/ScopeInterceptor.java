/*
 * $Id: ScopeInterceptor.java 520151 2007-03-19 22:48:16Z husted $
 *
 ...
 * <!-- END SNIPPET: example -->
 * 
 * 增加OgnlContextState.setCreatingNullObjects(ctx.getContextMap(), true)使得StaticParameters拦截器能够自动创建空对象
 * 
 * @author wangzh
 * 
 * @version 1.01
...
public class ScopeInterceptor extends AbstractInterceptor implements PreResultListener {
	...
	private static final long serialVersionUID = 2927825819712502630L;
	...
	/**
	 * Sets a list of aliases scoped properties
	 *
	 * @param s A comma-delimited list
	 */
	public void setAliases(String s) {
		if (s != null) {
			this.aliases = s.split("\\s*,\\s*");
		}
	}
	...
	private static class NULLClass implements Serializable {
		 ...
		 * 
		 */
		private static final long serialVersionUID = 1460695442235421861L;
			...
			for (int i = 0; i < application.length; i++) {
				...
				if (attribute != null) {
					...
					try{
						...
						if (aliases == null || aliases.length < i || StringUtils.isBlank(aliases[i])) {
							stack.setValue(string, nullConvert(attribute));
						}
						else {
							stack.setValue(aliases[i], nullConvert(attribute));
						}
					} finally {
	...
	public void beforeResult(ActionInvocation invocation, String resultCode) {
		...
		if (application != null)
			for (int i = 0; i < application.length; i++) {
				...
				Object value = null;
				if (aliases == null || aliases.length < i || StringUtils.isBlank(aliases[i])) {
					value = stack.findValue(string);
				}
				else {
					value = stack.findValue(aliases[i]);
				}
				if (LOG.isDebugEnabled()) {
